...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    float snoise3(vec3 p){const float K1=0.3333333;const float K2=0.1666667;vec3 i=floor(p+(p.x+p.y+p.z)*K1);vec3 d0=p-(i-(i.x+i.y+i.z)*K2);vec3 e=step(vec3(0.0),d0-d0.yzx);vec3 i1=e*(1.0-e.zxy);vec3 i2=1.0-e.zxy*(1.0-e);vec3 d1=d0-(i1-K2);vec3 d2=d0-(i2-K1);vec3 d3=d0-0.5;vec4 h=max(0.6-vec4(dot(d0,d0),dot(d1,d1),dot(d2,d2),dot(d3,d3)),0.0);vec4 n=h*h*h*h*vec4(dot(d0,hash33(i)),dot(d1,hash33(i+i1)),dot(d2,hash33(i+i2)),dot(d3,hash33(i+1.0)));return dot(vec4(31.316),n);}
  ...
  useEffect(() => {
    ...
    // --- Resize com debounce + DPR limitado
    let resizeTimeout;
    function resize() {
    ...
    const fps = 30; // limite de 30fps
    const frameInterval = 1000 / fps;

    const update = (t) => {
      rafId = requestAnimationFrame(update);
      if (!isVisibleRef.current) return; // pausa se não visível
      ...
      u.hover.value += (effectiveHover - u.hover.value) * 0.1;
      if (rotateOnHover && effectiveHover > 0.5)
        currentRot += (t - lastTime) * 0.0003;
  ...
  // Atualiza uniforms dinamicamente (sem recriar a cena)
  useEffect(() => {
    if (uniformsRef.current) {
      uniformsRef.current.hue.value = hue;
      uniformsRef.current.hoverIntensity.value = hoverIntensity;
    }
  }, [hue, hoverIntensity]);
